...
public class NewsFeedFragment extends Fragment implements LoaderManager.LoaderCallbacks<List<NewsFeedActivity>> {
    ...
    private ArrayList<NewsFeedActivity> newsList = new ArrayList<> ();
    ...
    public static NewsFeedFragment newInstance(String url) {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        if (isConnected () && newsList.isEmpty ()) {
            ...
            errorTextView.setText ( R.string.loading_fail_connection );
        }
    ...
    public void loadNextDataFromApi() {
        if (isConnected () && !newsList.isEmpty ()) {
            ...
            Toast.makeText ( getContext (), R.string.loading_fail_connection, Toast.LENGTH_SHORT ).show ();
        }
    }
        ...
        if (menuVisible) {
            if (recyclerView != null && adapter != null) {
                ...
                if (view != null) {
                    ...
                    imageView.startAnimation ( AnimationUtils.loadAnimation ( getContext (), R.anim.zoom ) );
                }
            }
        } else {
            if (recyclerView != null && adapter != null) {
    ...
    @NonNull
    @Override
    public Loader<List<NewsFeedActivity>> onCreateLoader(int id, @Nullable Bundle args) {
        return new NewsFeedLoader ( getContext (), NewsQueryUtils.createUrl ( uriParser ( url, page ) ) );
    }

    @Override
    public void onLoadFinished(@NonNull Loader<List<NewsFeedActivity>> loader, List<NewsFeedActivity> data) {
        hideLoading ();
        if (!data.isEmpty ()) {
            ...
            if (newsList.isEmpty ()) {
                errorTextView.setText ( R.string.home_screen_text ); // Default home screen message when there is internet
                errorTextView.setVisibility ( View.VISIBLE );
            }
        }
        View view = recyclerView.getLayoutManager ().findViewByPosition ( 0 );
    ...
    @Override
    public void onLoaderReset(@NonNull Loader<List<NewsFeedActivity>> loader) {
